...
export class ContatoCadastroComponent implements OnInit {
  ...
  listarFavoritos() {
    this.mensagens = [];
    this.contatoService.recuperarFavoritos()
      .then(
      (contatos) => {
        this.contatos = contatos;
      },
  ...
  favoritar(contato) {
    ...
    this.contatoService.favoritar(contato)
      .then(() => {
        this.carregar();
      },
      (erro) => {
        this.mensagens.push({ severity: 'error', summary: 'Erro', detail: 'Ocorreu um erro ao tentar favoritar o contato' });
      });
  }
}